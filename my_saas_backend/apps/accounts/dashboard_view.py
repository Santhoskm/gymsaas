...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        # PT clients: personal_training flag OR enrolled in a PT-type program
        pt_clients = clients.filter(
        ...
        # ── Revenue this month — using recognized_month ───────────────────────
        # recognized_month is always set to the first day of the target month
        # for add-ons, or to the expiry_date for base packages.
        # We filter by recognized_month falling within this calendar month.
        monthly_payments = Payment.objects.filter(
            gym=gym,
            recognized_month__gte=first_of_month,
            recognized_month__lt=next_month,
        )
        monthly_revenue = monthly_payments.aggregate(total=Sum("amount"))["total"] or 0

        # Split: new client revenue vs renewal / upgrade / add-on revenue
        new_client_ids = clients.filter(
        ...
        # ── Expenses this month ───────────────────────────────────────────────
        monthly_expenses = (
            Expense.objects.filter(gym=gym, date__gte=first_of_month, date__lt=next_month)
            .aggregate(total=Sum("amount"))["total"] or 0
        )
        ...
        for i in range(5, -1, -1):
            ...
            # New client revenue vs renewal / add-on revenue
            month_new_ids = clients.filter(
                join_date__gte=month_start, join_date__lt=month_end
            ).values_list('id', flat=True)
            month_new_rev = (
            ...
            revenue_chart.append(
                {
                    ...
                    "renewal_revenue": float(month_renewal_rev),
                    "expenses": float(month_exp),
                    "net_profit": float(month_rev) - float(month_exp),
                }
            )
        ...
        # ── PT breakdown per trainer ───────────────────────────────────────────
        from apps.trainers.models import Trainer
        pt_trainers = Trainer.objects.filter(
            gym=gym, offers_personal_training=True, status='active'
        ).prefetch_related('clients')

        pt_trainer_breakdown = []
        for trainer in pt_trainers:
            count = trainer.clients.filter(
                Q(personal_training=True) |
                Q(program_package__program__program_type='personal_training'),
                status='active',
            ).distinct().count()
            pt_trainer_breakdown.append({
                "trainer_id": trainer.id,
                "trainer_name": trainer.name,
                "pt_client_count": count,
            })
        ...
        return Response(
            {
                ...
                "revenue_chart": revenue_chart,
                "client_growth": client_growth,
                "pt_trainer_breakdown": pt_trainer_breakdown,
            }
        )