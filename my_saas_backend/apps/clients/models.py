...
class MembershipHistory(models.Model):
    ...
    recognized_month:
        ...
        - For new enrollment / renewal: set to expiry_date (the LAST month
          of that subscription period) so the full package revenue shows
          in the final month — not split across partial months.
        - For a mid-cycle add-on (e.g. personal training added in July):
          set to the date the add-on was purchased so it appears in the
          current month's revenue.
        - Defaults to the current date if not explicitly set.
    """
    ACTION_CHOICES = (
        ('new', 'New Enrollment'),
        ('renewal', 'Renewal'),
    ...
    def save(self, *args, **kwargs):
        ...
        # base packages count in the month they expire.
        if self.recognized_month is None:
            if self.action == 'addon':
                self.recognized_month = datetime.date.today()
            else:
