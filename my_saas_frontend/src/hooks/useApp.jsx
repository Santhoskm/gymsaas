...
export function AppProvider({ children }) {
  ...
  // Refetch whenever auth state changes — covers initial page load AND
  // logout→login as a different user without a full page refresh.
  // Previously this only ran once on mount ([] deps), so AppProvider (which
  // lives above the router and never unmounts) kept showing the *previous*
  // user's cached clients/trainers/dashboard data until a manual refresh
  // remounted everything from scratch.
  useEffect(() => {
    if (!isAuthenticated) {
      // Logged out — wipe cached data so it can never leak into the next session.
      setClients([]);
      setTrainers([]);
      setExpenses([]);
      setActivities([]);
